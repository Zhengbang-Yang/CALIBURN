...
class TextDataset(Dataset):
    def __init__(self, path, tokenizer, max_length):
        ...
        chunks = []
        n_full = len(input_ids) // max_length
        rem = len(input_ids) % max_length

        if n_full > 0:
            chunks.extend(input_ids[: n_full * max_length].view(n_full, max_length))

        if rem > 0:
            last = torch.full((max_length,), tokenizer.pad_token_id, dtype=torch.long)
            last[:rem] = input_ids[n_full * max_length:]
            chunks.append(last)

        if len(chunks) == 0:
            empty = torch.full((max_length,), tokenizer.pad_token_id, dtype=torch.long)
            chunks = [empty]

        self.input_ids = torch.stack(chunks, dim=0)  # [N, L]
        self.attention_mask = (self.input_ids != tokenizer.pad_token_id).long()  # [N, L]
